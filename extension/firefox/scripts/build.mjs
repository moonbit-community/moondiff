...
export function readBuildConfig(env = process.env, mode = "development") {
  if (!["development", "amo"].includes(mode)) {
...
function manifest(config) {
  ...
  return {
    ...
    content_scripts: [{
      // GitHub can SPA-navigate into a PR or commit without creating a new
      // document, so the route watcher must be present before that transition.
...
export function buildExtension({ env = process.env, log = process.stdout, mode = "development" } = {}) {
  ...
  try {
    ...
    for (const file of ["target.js", "content-script.js", "background.js"]) {
      copyFileSync(join(extensionRoot, "src", file), join(stagingRoot, file));
    }
    copyFileSync(
      join(extensionRoot, "icons", "favicon.svg"),
      join(stagingRoot, "icons", "favicon.svg"),
    );
    ...
    writeFileSync(
      ...
      `${JSON.stringify(manifest(buildConfig), null, 2)}\n`,
    );
    writeFileSync(
      join(stagingRoot, "config.js"),
      `globalThis.MoondiffConfig = Object.freeze(${JSON.stringify({
