...
ast_diff() {
  name=$1
  ./_build/native/release/build/moonbit-community/moondiff/moondiff/moondiff.exe "cli_test/source/${name}.old.mbt" "cli_test/source/${name}.new.mbt" >"cli_test/snapshot/${name}.txt"
}
