...
- [x] Refactor playground rendering to update individual file cards independently.
  - Affected areas: `playground/frontend/main/main.mbt`,
    `playground/frontend/internal/application/`,
    `playground/frontend/internal/rendering/`,
  ...
  - Reproduction: load [parser PR #183, commit 836b5e03](https://github.com/moonbitlang/parser/pull/183/changes/836b5e037954fee5e8b233aca8e3587621842875)
    ...
    Before the refactor, the user reported a temporary freeze in Edge. A Chromium fixture using the
  ...
  - [x] Define update boundaries for the page shell, toolbar, file tree,
  ...
  - [x] Replace the single whole-model `app_view` subscription with independent
  ...
  - [x] Move canonical patch parsing, review-row projection, inline discussion
  ...
  - [x] Reuse the existing diff/highlight caches within these boundaries and
  ...
  - [x] Scope sticky-layout observation and measurement to structural or size
  ...
  - [x] Add focused interaction regressions for expansion, section navigation,
  ...
  - [x] Add render/projection instrumentation to the browser regression fixture.
  ...
  - Verified: `rendering.spec.mjs` checks per-file view, canonical patch,
    review-row, projection, diff, highlight and VDOM counters, commit ordering,
    repeated navigation, and collection of retired DOM/source snapshots.
    The complete browser suite passes all 282 tests; `moon test --target all`
    passes on wasm, wasm-gc, JS and native. See the
    [rendering boundary and performance notes](playground/frontend/internal/rendering/README.md)
    for the normal/6x CPU comparison and the repeatable benchmark command.
...
- [ ] Keep changed-file navigation responsive at the supported 3,000-file limit.
  ...
  - [x] Cache the file-tree structure and avoid rebuilding every file card on
